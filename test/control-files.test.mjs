...
test("packageIdentityMismatch requires agreement across all three name slots", () => {
  ...
  assert.match(
    lockName,
    /package-lock\.json\.name is "other", but package\.json\.name is "consumer"/,
  );
...
test("packageChangesBeyondVersion only permits the version field", () => {
  ...
  assert.deepEqual(packageChangesBeyondVersion(before, changedDescription), [
    "description",
  ]);
  const removedKey = pkg("1.2.3");
  assert.deepEqual(packageChangesBeyondVersion(before, removedKey), [
    "description",
    "scripts",
  ]);
});
...
test("lockfileChangesBeyondVersion only permits the version fields", () => {
  ...
  assert.deepEqual(lockfileChangesBeyondVersion(before, changedDeps), [
    "dependencies",
  ]);
});
