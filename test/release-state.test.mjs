...
test("version unchanged is an ordinary push whatever files changed", () => {
...
test("valid release with no tag", () => {
...
test("tag already at the release commit is still a valid release", () => {
...
test("before SHA missing, malformed, all-zero, or unresolvable is a hard fail", () => {
  ...
  assert.match(
    verdict.reasons[0],
    /Found: missing, malformed, all-zero, or unresolvable\./,
  );
  assert.match(
    verdict.reasons[0],
    /Correction: re-run on a push whose previous SHA resolves to a commit\./,
  );
});

test("HEAD not equal to the triggering SHA is a hard fail", () => {
  const verdict = classifyRelease(validInput({ headMatchesTrigger: false }));
  assert.equal(verdict.verdict, "invalid");
  assert.match(
    verdict.reasons[0],
    /Checked: HEAD against the triggering SHA\./,
  );
});

test("new or previous version not stable X.Y.Z is a hard fail", () => {
  ...
  assert.match(
    previousUnstable.reasons[0],
    /Checked: the previous package\.json\.version\./,
  );
...
test("lockfile mismatch vs version is a hard fail", () => {
  ...
  assert.match(
    verdict.reasons[0],
    /Correction: regenerate the lockfile with npm install\./,
  );
...
test("package identity mismatch is a hard fail", () => {
  const verdict = classifyRelease(
    validInput({
      afterLock: {
        name: "other",
  ...
  assert.match(
    verdict.reasons[0],
    /Checked: package identity across package\.json and package-lock\.json\./,
  );
});

test("package.json changed beyond the version field is a hard fail", () => {
  const verdict = classifyRelease(
    validInput({
      afterPkg: { name: "consumer", version: "1.2.3", description: "changed" },
    }),
  );
  assert.equal(verdict.verdict, "invalid");
  assert.match(
    verdict.reasons[0],
    /Checked: package\.json changes beyond the version field\./,
  );
  assert.match(verdict.reasons[0], /Found: changed key\(s\): description\./);
  assert.match(
    verdict.reasons[0],
    /Correction: a release merge may only change package\.json\.version\./,
  );
});

test("package-lock.json changed beyond the version fields is a hard fail", () => {
  const verdict = classifyRelease(
    validInput({
      afterLock: {
        name: "consumer",
  ...
  assert.match(
    verdict.reasons[0],
    /Checked: package-lock\.json changes beyond the version fields\./,
  );
  assert.match(verdict.reasons[0], /Found: changed key\(s\): dependencies\./);
});

test("diff not exactly the three control files is a hard fail", () => {
  ...
  assert.match(
    extra.reasons[0],
    /Found: CHANGELOG\.md, package\.json, package-lock\.json, README\.md\./,
  );
...
test("changelog not a valid released version is a hard fail", () => {
...
test("tag at a commit other than the release commit is a hard fail", () => {
  ...
  assert.match(
    verdict.reasons[0],
    /Found: tag points at b{40}, not the triggering commit a{40}\./,
  );
  assert.match(
    verdict.reasons[0],
    /Correction: delete or move the tag to the release commit\./,
  );
});

test("ambiguous rows fail closed, never degrade to ordinary", () => {
