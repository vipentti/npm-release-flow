...
test("compareUrlFromRepoUrl derives the compare URL from repository.url", () => {
  ...
  assert.equal(
    compareUrlFromRepoUrl("https://gitlab.com/owner/repo.git"),
    null,
  );
});
...
test("prepare dry-run prints every would-mutation and changes nothing", async () => {
  ...
  try {
    ...
    assert.match(
      planText,
      /Signing preflight: commit-signing key [0-9a-fA-F]{40} available/,
    );
    ...
    assert.match(
      readConsumerFile(fixture, "CHANGELOG.md"),
      /## \[Unreleased\]/,
    );
...
test(
  ...
  async () => {
    ...
    try {
      ...
      const parents = git(["log", "-1", "--format=%P"], {
        cwd: fixture.consumer,
      })
        .stdout.trim()
        .split(/\s+/)
      ...
      assert.equal(
        parents[0],
        currentSha({ cwd: fixture.consumer, env })
    ...
    setGhPrs(fixture, "release/v1.2.3", [
      {
        number: 9,
        state: "MERGED",
        url: "https://github.com/example/fixture-consumer/pull/9",
      },
    ]);
    const err = await expectCliError(
      prepare(
    ...
    setGhPrs(fixture, "release/v1.2.3", [
      {
        number: 5,
        state: "CLOSED",
        url: "https://github.com/example/fixture-consumer/pull/5",
      },
    ]);
    const err = await expectCliError(
      prepare(
...
test("prepare exits 1 for a conflicting local branch only", async () => {
  ...
  try {
    ...
    assert.match(
      err.message,
      /Checked: the local release branch release\/v1\.2\.3\./,
    );
...
test("prepare exits 1 for a conflicting remote branch only", async () => {
  ...
  try {
    ...
    assert.match(
      err.message,
      /Checked: the remote release branch release\/v1\.2\.3\./,
    );
    assert.match(err.message, /Correction: resolve it manually/);
  } finally {
    fixture.cleanup();
...
test("prepare exits 1 for a missing or empty [Unreleased] section", async () => {
  ...
  try {
    ...
    assert.match(
      err.message,
      /Correction: add release notes under ## \[Unreleased\]/,
    );
  } finally {
    fixture.cleanup();
  }
...
test("prepare refuses when the configured signing key is not in the GPG keyring before any mutation", async () => {
  ...
  try {
    ...
    assert.match(
      err.message,
      /Checked: that the configured signing key f{40} is locally available in the GPG keyring\./,
    );
    assert.match(
      err.message,
      /Correction: import or restore the secret key in the local GPG home \(GNUPGHOME\)\./,
    );
    assert.equal(isCleanWorktree({ cwd: fixture.consumer }), true);
  } finally {
    fixture.cleanup();
...
test(
  ...
  async () => {
    ...
    try {
      ...
      const blockedEnv = {
        ...env,
        PATH: gitShim + (process.platform === "win32" ? ";" : ":") + env.PATH,
      };
      const err = await expectCliError(
        prepare(
          { version: "1.2.3", execute: true },
      ...
      assert.match(
        err.message,
        /Checked: pushing release\/v1\.2\.3 to origin\./,
      );
      assert.match(
        err.message,
        /Created: branch release\/v1\.2\.3, signed commit "release: 1\.2\.3"\./,
      );
      assert.match(
        err.message,
        /Remaining manual: push release\/v1\.2\.3 and create the release PR manually\./,
      );
    } finally {
      fixture.cleanup();
    }
